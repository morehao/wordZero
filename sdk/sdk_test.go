...
func TestContentRequestValidation(t *testing.T) {
	...
	t.Run("非空内容列表验证通过", func(t *testing.T) {
		req := &sdk.ContentRequest{
			Content: []sdk.ContentItem{
				{Type: sdk.ContentTypeParagraph, Text: "测试段落"},
			},
		}
		if err := req.Validate(); err != nil {
...
func TestContentTypes(t *testing.T) {
	if sdk.ContentTypeParagraph != "paragraph" {
		t.Errorf("ContentTypeParagraph 值错误: %s", sdk.ContentTypeParagraph)
	}
	if sdk.ContentTypeHeading != "heading" {
		t.Errorf("ContentTypeHeading 值错误: %s", sdk.ContentTypeHeading)
	}
	if sdk.ContentTypePageBreak != "page_break" {
		t.Errorf("ContentTypePageBreak 值错误: %s", sdk.ContentTypePageBreak)
	}
}
